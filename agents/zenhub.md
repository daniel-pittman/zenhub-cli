...
- `zh count [pipeline] [-q] [--json] [--no-verify]` — **exact** issue counts, taken from the API's own totalCount, and cross-checked against GitHub. **On `--json`, read `trustworthy` before `total`**: a count can be exact with respect to ZenHub and still wrong, because ZenHub's mirror of GitHub issue state goes stale when a workspace's sync lapses. `trustworthy: false` means ZenHub reports more open issues than GitHub has across the same repos (impossible unless the mirror is stale or the board holds ZenHub-only cards); `null` means the check could not run and is NOT agreement. `exact` keeps its original meaning — not a truncated page. `board` and `pipeline` carry the same caveat in their human output. Never report a count as fact when `trustworthy` is false. Use this whenever the answer is a number: a listing's length can be a truncated page, and a count that is quietly short is indistinguishable from a correct one. `-q` prints a bare number for scripting.
...
- **`--help` is safe on every subcommand.** It used to be bound as a free-form positional, so `zh create --help` created a real issue titled `--help`; three separate sessions did it in one day. It now prints usage. Unrecognized flags are refused rather than silently adopted as a title or silently dropped, so a typo fails loudly instead of producing a wrongly-named ticket. A title that genuinely starts with a dash requires `zh create -- "--like-this"`.
