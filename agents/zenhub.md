...
> Note: the example uses `create_issue`. The planning-noun creates (`epic_create` / `initiative_create` / `project_create` / `subtask_create`) also report parent-wire failure via `partial_applied` (v1.9.8), so the orphan guard above works through them too. Two signature differences if you substitute one in: they take `description=`, not `body=`, and they have no `type=` kwarg (the type is fixed per noun), so drop the `type=` argument from the call.
